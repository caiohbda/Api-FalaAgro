...
var newsController_exports = {};
__export(newsController_exports, {
  createNoticia: () => createNoticia,
  getNoticias: () => getNoticias,
  getNoticiasByState: () => getNoticiasByState
});
module.exports = __toCommonJS(newsController_exports);
...
// src/controllers/newsController.ts
var getNoticias = (request, reply) => __async(void 0, null, function* () {
  if (noticias.length === 0) {
    return reply.status(404).send({ message: "Nenhuma not\xEDcia dispon\xEDvel." });
  }
  return reply.send({ noticias });
});
var createNoticia = (request, reply) => __async(void 0, null, function* () {
  const { userId, title, content, state, city, image } = request.body;
  const newNoticia = {
